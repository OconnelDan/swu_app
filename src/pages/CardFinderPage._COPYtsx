...
function buildCandidates(
  ...
  for (const card of collectionCards) {
    candidates.set(card.cardId, { cardId: card.cardId, name: card.name ?? cardInfos.get(card.cardId)?.name });
  }
  for (const favorite of favorites) {
    for (const card of favorite.normalizedDeck.allRequiredCards) {
      if (!candidates.has(card.cardId)) {
        candidates.set(card.cardId, { cardId: card.cardId, name: cardInfos.get(card.cardId)?.name });
      }
    }
  }
...
export function CardFinderPage() {
  ...
  useEffect(() => {
    ...
    for (const card of collection?.cards ?? []) cardIds.add(card.cardId);
    for (const favorite of favorites ?? []) {
      for (const card of favorite.normalizedDeck.allRequiredCards) cardIds.add(card.cardId);
    }
    if (cardIds.size === 0) {
    ...
    cardProvider.getCards([...cardIds]).then(setCardInfos);
  }, [collection?.cards, favorites]);
  ...
  const results = normalizedQuery
    ? candidates
        .filter(
          (c) =>
            c.cardId.toLowerCase().includes(normalizedQuery) ||
    ...
    <div className="space-y-4">
      <section className="card space-y-2">
        ...
        <p className="text-xs text-slate-400">
          Busca por código (p. ej. LAW_038) o nombre para saber si tienes la carta libre, en qué
          mazo favorito está usada, o si no la tienes en tu colección.
        </p>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Código o nombre de la carta..."
            value={query}
      ...
      <ul className="space-y-2">
        {results.map((candidate) => {
          ...
          const imageUrl = info?.imageUrl ?? getCardImageUrl(candidate.cardId);
                ...
                <div className="flex-1">
                  ...
                  {status === "not_owned" ? (
                    ...
                    <p className="mt-1 text-sm">
                      ...
                      <strong className={allocation!.freeCount > 0 ? "text-saber-green" : "text-saber-red"}>
                  ...
                  {allocation && allocation.allocations.length > 0 && (
                    <ul className="mt-1 space-y-0.5 text-xs text-slate-400">
                      {allocation.allocations.map((a) => (
                        <li key={a.favoriteId}>
                          {a.usedCount}x usada en el mazo favorito «{a.favoriteName}»
                        </li>
                      ))}
                    </ul>
                  )}
