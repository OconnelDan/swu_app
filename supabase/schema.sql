...
create table if not exists public.favorite_decks (
  ...
  card_allocation_overrides jsonb not null default '{}'::jsonb,
  ...
  constraint favorite_decks_card_allocations_valid check (
...
alter table public.favorite_decks alter column card_allocation_overrides set not null;
...
begin
  ...
  if exists (
    ...
    from jsonb_to_recordset(p_favorite_decks) as deck(
      ...
      card_allocation_overrides jsonb
    )
    where deck.id is null
      ...
      or jsonb_typeof(coalesce(deck.card_allocation_overrides, '{}'::jsonb)) <> 'object'
      or exists (
        select 1
      ...
      or exists (
        select 1
        from jsonb_each(coalesce(deck.card_allocation_overrides, '{}'::jsonb))
      ...
      or (
        ...
        and (
          ...
          or coalesce(deck.card_allocation_overrides, '{}'::jsonb) <> '{}'::jsonb
        )
      )
  ) then
...
-- Añade nuevas copias sin sustituir la colección. El bloqueo por usuario
-- evita perder incrementos si se escanea desde dos dispositivos a la vez.
...
-- Resta copias de una carta y elimina su fila al llegar a cero. El bloqueo
-- por usuario serializa cambios realizados desde varios dispositivos.
...
-- Guarda el reparto exacto que el usuario ha elegido para una carta. Deben
-- enviarse todos los mazos montados que necesiten esa carta, incluidos los que
-- quedan con cero copias asignadas. La composición de los mazos no cambia.
